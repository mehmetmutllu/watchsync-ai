...
- [x] **CİLA (kullanıcı notları 2026-07-11):** (1) xfade tamamen kaldırıldı → hard-cut + TIMELINE bridge-dissolve, (2) mobil ölçek küçültüldü (üst %52 bölge), (3) telefon scrub BUG'ı kökten çözüldü: `<video>` → **canvas + WebP kare dizisi** (234 kare) + Next 16 `allowedDevOrigins` (LAN'da hydration hiç başlamıyormuş) (2026-07-11)
- [x] Akıcılık: segment içi smoothstep easing + ardışık kare alpha-harmanı; mobil track 700vh; black-crush+vignette (arka plan ışığı) + hero düz zemin (ızgara gizli); nav fixed; animasyon p=0.02'de başlar (2026-07-11)
- [x] **Landing yapı bölümleri (2026-07-11):** DAS PRODUKT ürün turu (BrowserShot mockup + gerçek DE ekranları; `ShowcaseSeeder` ile 12 saatlik vitrin verisi, APP_URL/storage:link fix'leri) · Pricing `#pricing` (99 € YER TUTUCU + Enterprise) · FAQ (5 soru, native details) · footer Impressum/Datenschutz + `/impressum` `/datenschutz` sayfaları (`[..]` yer tutucular)
- [ ] **MICROCOPY TURU (sıradaki — her dilde):** 12 maddelik jargon tablosu (mutex/SKU/Redis/LLM/API) + cta_title klişesi + Yetenekler 6→4 kart + tüm Landing metinleri DE ana dil; sonra dashboard $→€ + product ekranlarını yeniden çek
  - [x] ChatGPT'ye verilecek tek-parça microcopy brief'i hazırlandı (tüm anahtar+mevcut DE metni+jargon/klişe yasağı+`{key:{de,en,tr}}` çıktı formatı; 3 yeni anahtar `f1/f4/f5_meta` talebi) — kullanıcının JSON çıktısı bekleniyor (2026-07-14)
- [ ] **HERO VİDEO YENİDEN ÜRETİMİ (exploded-view) — strateji araştırıldı (2026-07-14):** Kök neden = Veo'ya patlamayı prompt'la icat ettirmek + ilk+son kare sabitlememek. **KARAR Yol A:** Nano Banana Pro keyframe (logosuz, kilitli, 5 kare) → Kling 2.5 Turbo Pro / Kling O1 start+end frame, segmentler durgun-poz bitişli → RIFE interpolate → WebP boru hattı. fal.ai ön ödemeli, ~$10 ücretsiz kredi, gerçekçi maliyet **$13-15** (öneri: $20 yükle). **Yol B yedek:** Blender + CGTrader "Watch 5 exploded" ($30-100, 240 kare hazır animasyon).
- [ ] Kullanıcıdan: gerçek fiyat (99 € ?) + Impressum/Datenschutz şirket bilgileri
