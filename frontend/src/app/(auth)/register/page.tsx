...
export default function RegisterPage() {
    ...
    try {
      ...
      if (axios.isAxiosError(err)) {
        if (err.response?.data?.errors) {
          ...
          setApiError(firstError?.[0] || "Kayıt başarısız.");
        } else {
          setApiError(err.response?.data?.message || "Kayıt başarısız. Lütfen tekrar deneyin.");
        }
      } else {
        setApiError("Kayıt başarısız. Lütfen tekrar deneyin.");
      }
    }
  };
  ...
  return (
    <div className="space-y-8">
      ...
      {/* Header */}
      <div>
        <h2 className="text-2xl font-bold text-primary-text">Hesap Oluştur</h2>
        <p className="mt-2 text-sm text-secondary-text">
          WatchSync AI&apos;a kayıt olarak envanter yönetimine başlayın.
        </p>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-5">
        ...
        <div className="space-y-2">
          ...
          >
            Şirket Adı{" "}
            <span className="text-disabled-text font-normal">(opsiyonel)</span>
          </label>
          <input
            id="company_name"
            type="text"
            placeholder="Luxury Watches Ltd."
