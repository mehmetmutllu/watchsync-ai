...
export default function AiStudioPage() {
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      const message = err?.response?.data?.message || 'AI servisi şu anda kullanılamıyor. Lütfen tekrar deneyin.';
      setError(message);
      setStep('upload');
    } finally {
          ...
          <div className="glass rounded-xl p-6">
            ...
            {step === 'processing' && (
              <div className="flex flex-col items-center justify-center gap-4 py-20">
                ...
                <div className="text-center">
                  <p className="text-sm font-medium text-primary-text">Görsel işleniyor...</p>
                  <p className="mt-1 text-xs text-secondary-text">
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-1 gap-4 xl:gap-6">
          ...
          {result && result.results.length > 1 && (
            <div className="glass rounded-xl p-5 space-y-3">
              <h3 className="text-sm font-semibold text-primary-text">Varyantlar</h3>
          ...
          {/* AI Info */}
          <div className="glass rounded-xl p-5 space-y-3">
            <h3 className="text-sm font-semibold text-primary-text">AI Hakkında</h3>
            <div className="space-y-2 text-xs text-secondary-text">
              <p>
                <span className="font-medium text-primary-text">SAM 2</span> modeli ile otomatik
                saat segmentasyonu yapılır.
              </p>
              <p>
                Arka plan kaldırılır ve seçtiğiniz profesyonel arka plan ile değiştirilir.
              </p>
